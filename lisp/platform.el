...
(defun platform-gui-p ()
  "Return non-nil if running in GUI mode.
This is a function rather than a constant to support daemon mode,
where GUI status can change when clients connect."
  (display-graphic-p))

(defun platform-terminal-p ()
  "Return non-nil if running in terminal mode.
This is a function rather than a constant to support daemon mode,
where terminal status can change when clients connect."
